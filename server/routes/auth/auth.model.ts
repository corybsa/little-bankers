...
export const login = async (req: Request, res: Response, next: Function) => {
  try {
    ...
    AuthHelper.verifyPassword(req.body.password, user.hash!, async (err: Error, success: boolean) => {
      if(err) {
        next(err, null);
      } else if(!success) {
...
export const register = async (req: Request, next: Function) => {
  try {
    ...
    AuthHelper.hashPassword(req.body.password, async (err: Error, hash: string) => {
