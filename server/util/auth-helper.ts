...
class AuthHelper {
  ...
  createSessionCookie(user: IUserSchema, req: Request, res: Response, next: Function) {
    ...
    this.hashPassword(validator, async (err: Error, hash: string) => {
   ...
   */
  hashPassword(password: string, callback: (err: Error, hash: string) => void) {
   ...
   */
  verifyPassword(password: string, hash: string, callback: (err: Error, success: boolean) => void) {
